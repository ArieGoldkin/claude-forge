...
process.on("uncaughtException", (error) => {
  try {
    process.stderr.write(`[ai-toolkit-hooks] Uncaught: ${error.message}
`);
  } catch {
  }
  try {
...
process.on("unhandledRejection", (reason) => {
  try {
    ...
    process.stderr.write(`[ai-toolkit-hooks] Unhandled: ${msg}
`);
  } catch {
  }
  try {
...
async function main() {
  ...
  try {
    ...
    console.error(`[ai-toolkit-hooks] Error in ${hookName}: ${message}`);
    outputResult(silentSuccess());
    process.exit(0);
  }
}
...
if (isMainModule) {
  main().catch((error) => {
    const message = error instanceof Error ? error.message : "Unknown error";
    console.error(`[ai-toolkit-hooks] Fatal error: ${message}`);
