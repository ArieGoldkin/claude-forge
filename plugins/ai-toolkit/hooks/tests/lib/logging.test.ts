...
describe('logging module', () => {
  ...
  describe('getCurrentLogLevel', () => {
    it('should default to warn when AI-TOOLKIT_LOG_LEVEL is not set', () => {
      delete process.env['AI-TOOLKIT_LOG_LEVEL'];
      resetLogLevel();
      expect(getCurrentLogLevel()).toBe('warn');
    });

    it('should return debug when AI-TOOLKIT_LOG_LEVEL=debug', () => {
      process.env['AI-TOOLKIT_LOG_LEVEL'] = 'debug';
      resetLogLevel();
      expect(getCurrentLogLevel()).toBe('debug');
    });

    it('should return info when AI-TOOLKIT_LOG_LEVEL=info', () => {
      process.env['AI-TOOLKIT_LOG_LEVEL'] = 'info';
      resetLogLevel();
      expect(getCurrentLogLevel()).toBe('info');
    });

    it('should return warn when AI-TOOLKIT_LOG_LEVEL=warn', () => {
      process.env['AI-TOOLKIT_LOG_LEVEL'] = 'warn';
      resetLogLevel();
      expect(getCurrentLogLevel()).toBe('warn');
    });

    it('should return error when AI-TOOLKIT_LOG_LEVEL=error', () => {
      process.env['AI-TOOLKIT_LOG_LEVEL'] = 'error';
      resetLogLevel();
      expect(getCurrentLogLevel()).toBe('error');
    });

    it('should default to warn for invalid log level', () => {
      process.env['AI-TOOLKIT_LOG_LEVEL'] = 'invalid';
      resetLogLevel();
      expect(getCurrentLogLevel()).toBe('warn');
    });
  ...
  describe('getLogDir', () => {
    it('should return path under HOME/.claude/logs/ai-toolkit', () => {
      const logDir = getLogDir();
      expect(logDir).toContain('.claude');
      expect(logDir).toContain('logs');
      expect(logDir).toContain('ai-toolkit');
      expect(logDir.endsWith('ai-toolkit')).toBe(true);
    });
