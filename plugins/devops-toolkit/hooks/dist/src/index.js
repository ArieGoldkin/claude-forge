...
import { readSync, existsSync, readFileSync, mkdirSync, readdirSync, rmdirSync } from 'fs';
...
var HOOK_NAME = "repo-access-guard";
var POLICY_FILENAME = "repo-access-policy.json";
function loadPolicy() {
  const candidates = [
    join(homedir(), ".claude", POLICY_FILENAME),
    ...process.env["CLAUDE_PLUGIN_ROOT"] ? [join(process.env["CLAUDE_PLUGIN_ROOT"], ".claude", POLICY_FILENAME)] : []
  ];
...
async function repoAccessGuard(_input) {
...
var HOOK_NAME2 = "continuity-recommendation";
var CONTINUITY_NAMES = ["ctk", "continuity-toolkit"];
function isContinuityInstalled() {
  const pluginRoot = process.env["CLAUDE_PLUGIN_ROOT"];
  if (!pluginRoot) {
    logDebug(HOOK_NAME2, "CLAUDE_PLUGIN_ROOT not set, skipping check");
    return true;
  }
  ...
  for (const name of CONTINUITY_NAMES) {
    ...
    if (existsSync(marketplacePath)) {
      logDebug(HOOK_NAME2, `Found continuity plugin at ${marketplacePath}`);
      return true;
    }
  }
  ...
  for (const name of CONTINUITY_NAMES) {
    ...
    if (existsSync(siblingPath)) {
      logDebug(HOOK_NAME2, `Found continuity plugin at ${siblingPath}`);
      return true;
    }
  }
  logDebug(HOOK_NAME2, `continuity plugin (ctk) not found at ${marketplaceDir} or ${parentDir}`);
  return false;
}
function claimRecommendationMarker() {
  ...
  try {
    ...
    logDebug(HOOK_NAME2, `Claimed recommendation marker: ${markerDir}`);
    return true;
  } catch (err) {
    const code = err.code;
    if (code === "EEXIST") {
      logDebug(HOOK_NAME2, "Recommendation already shown by another plugin");
    } else {
      logDebug(HOOK_NAME2, `Failed to create marker: ${code}`);
    }
    return false;
  }
}
...
registerHook(
  "lifecycle/repo-access-guard",
  "Block non-Bedrock users from restricted repositories",
  repoAccessGuard
);
registerHook(
