...
describe('logging module', () => {
  ...
  describe('getCurrentLogLevel', () => {
    it('should default to warn when DEVOPS_LOG_LEVEL is not set', () => {
      delete process.env['DEVOPS_LOG_LEVEL'];
      resetLogLevel();
      expect(getCurrentLogLevel()).toBe('warn');
    });

    it('should return debug when DEVOPS_LOG_LEVEL=debug', () => {
      process.env['DEVOPS_LOG_LEVEL'] = 'debug';
      resetLogLevel();
      expect(getCurrentLogLevel()).toBe('debug');
    });

    it('should return info when DEVOPS_LOG_LEVEL=info', () => {
      process.env['DEVOPS_LOG_LEVEL'] = 'info';
      resetLogLevel();
      expect(getCurrentLogLevel()).toBe('info');
    });

    it('should return warn when DEVOPS_LOG_LEVEL=warn', () => {
      process.env['DEVOPS_LOG_LEVEL'] = 'warn';
      resetLogLevel();
      expect(getCurrentLogLevel()).toBe('warn');
    });

    it('should return error when DEVOPS_LOG_LEVEL=error', () => {
      process.env['DEVOPS_LOG_LEVEL'] = 'error';
      resetLogLevel();
      expect(getCurrentLogLevel()).toBe('error');
    });

    it('should default to warn for invalid log level', () => {
      process.env['DEVOPS_LOG_LEVEL'] = 'invalid';
      resetLogLevel();
      expect(getCurrentLogLevel()).toBe('warn');
    });
  ...
  describe('getLogDir', () => {
    it('should return path under HOME/.claude/logs/devops', () => {
      const logDir = getLogDir();
      expect(logDir).toContain('.claude');
      expect(logDir).toContain('logs');
      expect(logDir).toContain('devops');
      expect(logDir.endsWith('devops')).toBe(true);
    });
