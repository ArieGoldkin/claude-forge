...
| 9 | **Parallel Collision** (S2) | Two sub-agents edit the same files; merge conflicts; corrupt state | `Workflow` `isolation: 'worktree'` for parallel file-mutating agents + ctk worktree-create/remove lifecycle; **structurally**, every etk agent excludes `Agent`/`Task`, so a routed skill cannot spawn colliding children (root CLAUDE.md) | structural + worktree |
