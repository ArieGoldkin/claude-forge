...
10. **Sentry ID present → triage, overriding BOTH `fix` and `diagnose`.** Three categories collide here and `triage` is listed last, so a bare keyword match never reaches it: `fix` is **row 1** and owns "error" — which sits inside `triage`'s own signal "production error" — and `diagnose` owns "investigate". **The ID is the tiebreak, not the verb.** "Triage the production error in checkout, SENTRY PROJ-4F2" → `triage` (an ID is present), *not* `fix` despite "error" matching first. "Investigate PROJ-4F2" / "triage this sentry issue {url}" → `triage`. "Fix the production error" (no ID) → `fix`. "Investigate why checkout crashes" (no ID) → `diagnose`. Rationale: `investigate-sentry` needs a resolvable issue reference to fetch; without one it has nothing to open, and `/fix-bug`'s observation loop is the right tool anyway.
...
12. **Opening an MR/PR → ship; reviewing an existing one → review.** This is the widest collision in the table: `review` sits **6 rows before** `ship` and owns "review, MR, PR, merge request, pull request" — substrings of nearly every `ship` signal ("open a **PR**", "**PR** description", "create a **pull request**", "ready for **review**"). Only "ship it" is unambiguous. **The distinguishing question is whether the MR/PR already exists.** "Ship it" / "ready for review" / "open a PR" / "create a pull request" → `ship` (`/prepare-pr` — authors the description and *creates* it). "Review !42" / "look at PR 26" (an existing number) → `review` (`/review-mr` — reviews one that exists; it never creates one). Heuristic: an MR/PR **number** in the goal → `review`; no number, and the user wants one to exist → `ship`.
13. **HIPAA/PHI term present → compliance, not verify.** `verify` owns "check / validate / ensure" and sits earlier, so "check if we're HIPAA compliant" matches `verify` on "check" first. Any of HIPAA / PHI / BAA / protected health information / data privacy → `compliance` (`/hipaa-compliance-checker`). Plain "check that tests pass" stays `verify`.
