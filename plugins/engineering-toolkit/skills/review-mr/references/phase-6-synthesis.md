...
After Phase 6, proceed to Phase 7 (artifact writing) **when there are findings to post**. But honor the **return contract** (SKILL.md § Output): this skill runs `context: fork`, so the **synthesized Phase-6 report MUST be your final message** — the caller sees only your last message. Write any artifacts and the post hand-off *first*, then re-emit the Phase-6 report as your final output. **Never end on the terse Phase-7c hand-off block, and never end with only "completed"** — a bare completion returns nothing to the caller and is indistinguishable from the skill being dead (this regression shipped twice). With **zero findings** — a clean, low-risk PR is a common case — skip artifact-writing entirely and return the report directly: an empty Concerns section plus a SHIP/APPROVE recommendation is a complete, valid review.
